...
class DualSoftMaxMatcher(torch.nn.Module):
    ...
    def forward(
        self, keypoints_A, descriptions_A, keypoints_B, descriptions_B, normalize=False, inv_temp=1, threshold=0.0
    ):
        if isinstance(descriptions_A, list):
            matches = [
                ...
                for k_A, d_A, k_B, d_B in zip(keypoints_A, descriptions_A, keypoints_B, descriptions_B)
            ]
        ...
        inds = torch.nonzero(
            (P == P.max(dim=-1, keepdim=True).values) * (P == P.max(dim=-2, keepdim=True).values) * (P > threshold)
        )
        batch_inds = inds[:, 0]
...
class DeDoDe_baseline(BaseExtractor):
    ...
    def match(self, image1, image2):
        ...
        matches_A, matches_B, batch_ids = self.matcher(
            kp0, desc0, kp1, desc1, normalize=True, inv_temp=20, threshold=0.1
        )
