...
class ALIKED(torch.nn.Module):
    ...
    def __init__(self, conf={}):
        ...
        if conf.model_name == "aliked-n":
            conf.model_name = "aliked-n16"
    ...
    def forward_desc(self, data, keypoints):
        ...
        # keypoints: BxNx2 (w,h) -> BxNx2 (-1+1)
        keypoints = 2.0 * keypoints / wh - 1

        feature_map, scoremap = self.extract_dense_map(image)
        descriptors, offsets = self.desc_head(feature_map, keypoints)
        scores = torch.nn.functional.grid_sample(
            scoremap, keypoints.view(b, -1, 1, 2), mode="bilinear", align_corners=True
        ).squeeze()
        
        keypoints = (keypoints + 1) / 2 * wh  # BxNx2 (-1+1) -> BxNx2 (w,h)
        
        return {
            "descriptors": torch.stack(descriptors),  # B N D
            "keypoints": keypoints,  # B N 2
            "keypoint_scores": scores,  # B N
        }
